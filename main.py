...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Удаление ссылок вида @username или @canal
    if "@" in text:
        try:
            await update.message.delete()
            user_warnings[user_id] += 1
            ...
            if user_warnings[user_id] == 1:
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"⚠️ @{user.username or user.first_name}, los enlaces con @ no están permitidos. Próxima vez = mute."
                )
            elif user_warnings[user_id] >= 2:
                until = datetime.now() + timedelta(hours=24)
                await context.bot.restrict_chat_member(
                ...
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"🚫 @{user.username or user.first_name} fue silenciado por 24 horas por enviar enlaces con @ nuevamente."
                )
        except Exception as e:
